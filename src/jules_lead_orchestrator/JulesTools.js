...
async function handleKillSubAgent(args, confirmToolCall) {
  try {
...
async function handlePauseSubAgent(args, confirmToolCall) {
...
async function handleReassignModule(args, confirmToolCall) {
  try {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 15000);
...
async function handleBroadcastUpdate(args, confirmToolCall) {
...
async function handleSetAgentDependency(args, confirmToolCall) {
  upsertSession({
...
async function handleCreateSharedContract(args, confirmToolCall) {
...
async function handleGenerateInkTerminalDiagram(args, confirmToolCall) {
...
async function handleDispatchSubAgent(args, confirmToolCall, orchestratorSessionId) {
  try {
...
async function handleMergeBranches(args, confirmToolCall, orchestratorSessionId) {
...
export async function handleOrchestratorToolCall(toolCall, orchestratorSessionId) {
  ...
  switch (name) {
    case 'kill_sub_agent':
      return await handleKillSubAgent(args, confirmToolCall);
    case 'pause_sub_agent':
      return await handlePauseSubAgent(args, confirmToolCall);
    case 'reassign_module':
      return await handleReassignModule(args, confirmToolCall);
    case 'broadcast_update':
      return await handleBroadcastUpdate(args, confirmToolCall);
    case 'set_agent_dependency':
      return await handleSetAgentDependency(args, confirmToolCall);
    case 'create_shared_contract':
      return await handleCreateSharedContract(args, confirmToolCall);
    case 'generate_ink_terminal_diagram':
      return await handleGenerateInkTerminalDiagram(args, confirmToolCall);
    case 'dispatch_sub_agent':
      return await handleDispatchSubAgent(args, confirmToolCall, orchestratorSessionId);
    case 'merge_branches':
      return await handleMergeBranches(args, confirmToolCall, orchestratorSessionId);
    default:
