...
> Read your requirements file first. It tells you exactly who you are, what folder domains you own, and what you are not allowed to touch.
...
1. **NEVER touch out-of-domain files:** If you are the TUI agent, do NOT edit files inside `src/state/` or `src/queue/`. If a bug exists in another domain, you MUST leave it alone and write a message to that agent's inbox. Cross-domain edits result in instant code review failure.
...
|---|---|---|
| **Executive** | `agent_roles/AGENT_EXECUTIVE_REQUIREMENTS.md` | **God Mode:** Can change any file it wants. Reviews work, enforces inbox rules, and writes product code across all domains. |
| **Config** | `agent_roles/AGENT_CONFIG_REQUIREMENTS.md` | **Settings:** Owns all configuration files and `jorch config *` commands (`src/cli/` ALL FILES). |
| **Conflict** | `agent_roles/AGENT_CONFLICT_REQUIREMENTS.md` | **Resolver:** Owns `jorch conflict` commands and dedicated merge session tools (`src/conflict/` ALL FILES). |
| **Decomposer** | `agent_roles/AGENT_DECOMPOSER_REQUIREMENTS.md` | **Parser:** Splits raw prompts into atomic, typed tasks (`src/decomposer/` ALL FILES). |
| **Jules Lead Orchestrator BUILDER** | `agent_roles/AGENT_JULES_LEAD_ORCHESTRATOR_REQUIREMENTS.md` | **Master Dispatcher BUILDER:** Makes the Hybrid brain so that it can analyze user intent, provide tools, and orchestrate tasks between user and sub-agents (`src/jules_lead_orchestrator/` ALL FILES). |
| **Queue** | `agent_roles/AGENT_QUEUE_REQUIREMENTS.md` | **Traffic Control:** Enqueues, sorts priority, checks file locks (`src/queue/` ALL FILES). |
| **State** | `agent_roles/AGENT_STATE_REQUIREMENTS.md` | **Source of Truth:** Owns persistent store and Jules API calls (`src/state/` ALL FILES). |
| **TUI** | `agent_roles/AGENT_TUI_REQUIREMENTS.md` | **Dashboard + Entry Point:** Owns terminal render loop, UI formatting, and CLI wiring (`src/tui/` ALL FILES, `bin/` ALL FILES). |
...
6. **Stay in your lane:** Only edit files inside your designated folder Domains.
